...
$html='
<div class="title2">Upload Plugin</div>
<div class="plugin_content">

<form action="upload.php" method="post" enctype="multipart/form-data">
      <input type="text" name="returnsto" value="plugins" style="display: none;" />
      <div>
      ...
      <div>
            ...
            <input id="fileDownload" type="submit" name="submit" value="Download" style="margin-left: 5px; margin-right: 5px;" onClick="$(\'#urlUploadKind\').attr(\'checked\', true);" />
      </div>
</form>';
   ...
   if ((exec("ls upload | wc -l") == 2) && file_exists('upload/log'))
   {
         ...
         <div id='resultsOfLoad'>
            ...
            if(exec("ls upload | grep libplg | wc -l"))
            {
                  $nameFile = exec("ls upload | grep libplg");
                  $html .= "<button onclick='installPlugin(\"".$section."\",\"".$plugin."\",\"".$nameFile."\")' >Install Plugin</button>";
            }
            elseif(exec("ls upload | grep libupd | wc -l"))
            {
                  $html .= "<b>Update package detected</b>";
            }
            $html .=
         "</div>";
   }
...
$html.='
<div class="title2">Check for Plugins</div>
<div class="plugin_content">';
$html.="<button onclick='checkForPlugins(\"".$section."\",\"".$plugin."\")' >Check for plugins</button>";
