...
function saveInterfaces($interface,$post,$read_path,$write_path) {
    ...
    switch ($interface) {
        ...
        // We will have to manage the mode of wlanconfig in function of the mode choosen.
            ...
            if ($post['essid']) {
                $input[$interface]['pre-up']['iwconfig']['essid']=$post['essid'];
            }
            else {
                unset($input[$interface]['pre-up']['iwconfig']['essid']);
            }
            ...
            if ($post['mac_essid_i']) {
                $input[$interface]['pre-up']['iwconfig']['ap']=$post['mac_essid_i'];
            }
            else {
                unset($input[$interface]['pre-up']['iwconfig']['ap']);
            }

            // Select fields allways has a value in it.
            $input[$interface]['pre-up']['iwconfig']['mode']=$post['mode'];


            if ($post['freq']=='2') {
                $input[$interface]['pre-up']['iwconfig']['channel']=$post['channel2'];
                $input[$interface]['up']['iwpriv']['mode']=$post['mode-abg'];
            }
            else {
                $input[$interface]['pre-up']['iwconfig']['channel']=$post['channel5'];
                $input[$interface]['up']['iwpriv']['mode']='3';
            }

            $input[$interface]['pre-up']['iwconfig']['txpower']=$post['tx_power'];


            // FRAGMENTATION
            if (($post['frag']=='off')||(($post['frag']>'255')&&($post['frag']<'2347'))) {
                $input[$interface]['pre-up']['iwconfig']['frag']=$post['frag'];
            }
            else {
                unset($input[$interface]['pre-up']['iwconfig']['frag']);
            }
            ...
            if ($post['rate']=='auto') {
                $input[$interface]['pre-up']['iwconfig']['rate']=$post['rate'];
            }
            else {
                $input[$interface]['pre-up']['iwconfig']['rate']=$post['rate_val'];
            }
