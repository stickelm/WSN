...
$ini = file( $filepath );
    ...
    foreach( $ini as $line )
    {
        ...
        elseif(($data[0]=='address')&&($last_interface!='none')&&($selip6==0)){
            // we save address data
            $configInterfaces[$last_interface]['address']=$data[1];
        }
        elseif(($data[0]=='netmask')&&($last_interface!='none')&&($selip6==0)){
            // we save netmask data
            $configInterfaces[$last_interface]['netmask']=$data[1];
        }
        elseif(($data[0]=='gateway')&&($last_interface!='none')&&($selip6==0)){
        ...
        elseif(($data[0]=='address')&&($last_interface!='none')&&($selip6==1)){
            // we save address data
            $configInterfaces[$last_interface]['address6']=$data[1];
        }
        elseif(($data[0]=='netmask')&&($last_interface!='none')&&($selip6==1)){
            // we save address data
            $configInterfaces[$last_interface]['netmask6']=$data[1];
        }
        elseif(($data[0]=='gateway')&&($last_interface!='none')&&($selip6==1)){
            // we save address data
            $configInterfaces[$last_interface]['gateway6']=$data[1];
        }
        ...
        elseif((($data[0]=='pre-up')||($data[0]=='up')||($data[0]=='post-up')||($data[0]=='pre-down')||($data[0]=='down')||($data[0]=='post-down'))&&($last_interface!='none')){
            ...
            elseif (($data[0]=='pre-up')&&$data[1]=='iwconfig')
            {
            	$opciones=explode(' ',$data[3]);
