 ...
 */
function save_gprs($post,$gprs_usb='ttyUSB2',$write_path='')
{
    global $base_plugin;
	...
	$write_pin_path=$base_plugin.'data/wvdial_pin.conf';
    ...
    // In meshlium the pin is saved to an auxiliary function but the wvdial command
    // is preserved (commented) to port it easily.
    // meshlium save
    $fp_pin=fopen($write_pin_path,"w");
    fwrite($fp_pin,"#!/bin/bash\npin=\"".$post['PIN']."\"\n");
    fclose($fp_pin);
    /* wvdial save.
    if ($post['PIN'])
    {
