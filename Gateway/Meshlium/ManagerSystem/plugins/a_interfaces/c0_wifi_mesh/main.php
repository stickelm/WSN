...
include_once $API_core.'../msh_init_conf.php';
...
if (count($ifaces) == 1)
{
    $html .= "<h3 class='alarm'>No Wifi Detected</h3><br><br>";
}*/

/*manu*
if (in_array("ath0", $init_ifaces))
{
    if (!in_array('ath0', $a))
    {
        $html .= "<h3 class='alarm'>Problems detected in Wifi0<br><br></h3>";
    }

}
if (in_array("ath1", $init_ifaces))
{
    if (!in_array('ath1', $a))
    {
        $html .= "<h3 class='alarm'>Problems detected in Wifi1<br><br></h3>";
    }
...
$html .= "<div class='cleaner'></div>";
    $html.= "<div class='hidden'>";
        $html.=make_selector_arg($iload);
    $html.= "</div>";

    exec("cat /mnt/lib/cfg/currentPreset", $currentPreset);
    exec("cat /mnt/lib/cfg/initialConf", $initConf);

    if(!strstr($initConf['0'], "wifiMesh"))
    {
        $html.= '<b>The "Wifi Mesh" radio is not instaled</b>';
    }
    elseif(($currentPreset['0'] == "Meshlium AP") || ($currentPreset['0'] == "Meshlium GPRS AP") || ($currentPreset['0'] == "Meshlium Zigbee AP") || ($currentPreset['0'] == "Meshlium Zigbee GPRS AP"))
    {
        $html.= '<b>Please go to the Presets section and select a Mesh preset </b>';
    }
    else
    {
        $html.=make_wireless('/etc/network/interfaces','ath1',false);
    }

//$html.=make_selector($ifaces);
