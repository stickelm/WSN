...
function make_inputs($hcid_conf)
{
    ...
    exec("cat /etc/bluetooth/hcid.conf | grep power", $BtPower);
    $list.='
        <div>
                ...
                <table style="float: left;" id="rounded-corner">
                    <tbody>
                        <tr>
                            ...
                            <td>
                                <input type="text" class="ms_alnum ms_mandatory" id="name" name="name" value="'.$BtName['0'].'" maxlength="16" />
                            </td>
                            <td><div id="name_ms_cte" class="ms_cte"></div></td><td></td>
                        </tr>
                        <tr>
                            <td>
                                <span>Visible</span>
                            ...
                            <td>
                                <input name="visible" type="checkbox" ';
                                    if(strstr($BtVisible['0'], "enable"))
                                    {
                                        $list.='checked';
                                    }
                                    $list.='
                                /><span style="line-height:19px;margin-left:3px;vertical-align:top;">Activate</span>
                            </td><td></td><td></td>
                        </tr>
                        <tr>
                            <td>
...
/*
	...
	if (strstr($BtPower[0],'High'))
	    {
	    $list.="<option selected=\"yes\" value=High>High</option>";
	    $list.="<option value=Medium>Medium</option>";
	    $list.="<option value=Low>Low</option>";
	    $list.="</select></td>";
	    }
	else if (strstr($BtPower[0],'Medium'))
	    {
	    $list.="<option selected=\"yes\" value=Medium>Medium</option>";
	    $list.="<option value=High>High</option>";
	    $list.="<option value=Low>Low</option>";
	    $list.="</select></td>";
	    }
	else if (strstr($BtPower[0],'Low'))
	    {
	    $list.="<option selected=\"yes\" value=Low>Low</option>";
	    $list.="<option value=High>High</option>";
	    $list.="<option value=Medium>Medium</option>";
	    $list.="</select></td>";
	    }
	    ...
	    $list.="</select></td>";
             ...
             $list.='<td><div id="name_ms_cte" class="ms_cte"></div></td><td></td>
                        </tr>*/
 $list.='                   </tbody>
                </table>';
...
if (exec("ps ax | grep BtScan | grep -v grep | wc -l") == 1)
{
   $list.= '<div id="dRunning"></div> <span> <b>Bluetooth Scan Available</b></span>';
}
elseif (exec("ps ax | grep BtScan | grep -v grep | wc -l") == 0)
{
   $list.= '<div id="dStopped"></div> <span> <b>Bluetooth Scan Not Available</b></span>';
}
else
{
...
function make_storedData()
{
	...
	<div style="height:300px;width:97%;overflow: scroll;font-family:monospace; background-color:white; -moz-border-radius:5px;background-color:white;border:1px solid #898989;margin:15px 0 0 0;overflow:auto;padding:10px;">
	  <div id="fileViewer" style="height:300px;width:900px;"></div>
	</div>
';
    return $list;
}
...
function make_storedDataLocalDB()
{
    ...
    <div style="clear: both;"></div>
	<div style="height:300px;width:97%;overflow: scroll;font-family:monospace; background-color:white; -moz-border-radius:5px;background-color:white;border:1px solid #898989;margin:15px 0 0 0;overflow:auto;padding:10px;">
	  <div id="localDataViewer" style="height:300px;width:800px;"></div>
	</div>
';
...
function make_storedDataExtDB()
{
    ...
    <div style="clear: both;"></div>    
	<div style="height:300px;width:97%;overflow: scroll;font-family:monospace; background-color:white; -moz-border-radius:5px;background-color:white;border:1px solid #898989;margin:15px 0 0 0;overflow:auto;padding:10px;">
	  <div id="extDataViewer" style="height:300px;width:800px;"></div>
	</div>
';
...
function make_interface()
{
    ...
    $list.='<div class="title2">Bluetooth scan</div>';
