 ...
 *
 *  Version 0.1
 *  Author: Octavio Benedi <o[dot]benedi[at]libelium[dot]com>
 */
...
var error = new Array();
    ...
    error['ms_30']='must be < 30';
    error['ms_60']='must be > 60';
/*
...
function test_element(el)
{
    ...
    if($(el).attr('name'))
    {
        ...
        if($(el).hasClass("ms_60"))
        {
            if($(el).val()<60)
            {
                error_detected=true;
                set_alert($(el).attr('id'),'ms_60');
            }
        }
        if($(el).hasClass("ms_30"))
        {
            if($(el).val()>30)
            {
                error_detected=true;
                set_alert($(el).attr('id'),'ms_30');
            }
        }
    }
    return error_detected;
...
//Control marcado inpusts always y firsttime
$("#firsttime").click(function() { if ($(this).attr("checked")) $("#always").removeAttr("checked")});
$("#always").click(function() { if ($(this).attr("checked")) $("#firsttime").removeAttr("checked")});
