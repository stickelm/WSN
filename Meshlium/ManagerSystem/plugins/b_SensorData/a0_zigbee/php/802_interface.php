...
function _802_interface()
{
    ...
    $NetworkID = exec("cat ".$base_plugin."data/802.conf | grep NetworkID | cut -d':' -f2");
    $Channel = exec("cat ".$base_plugin."data/802.conf | grep Channel | cut -d':' -f2");
    $NetworkAddress = exec("cat ".$base_plugin."data/802.conf | grep NetworkAddress | cut -d':' -f2");
    $NodeID = exec("cat ".$base_plugin."data/802.conf | grep NodeID | cut -d':' -f2");
    $PowerLevel = exec("cat ".$base_plugin."data/802.conf | grep PowerLevel | cut -d':' -f2");
    $EncryptedMode = exec("cat ".$base_plugin."data/802.conf | grep EncryptedMode | cut -d':' -f2");
    $EncryptKey = exec("cat ".$base_plugin."data/802.conf | grep EncryptKey | cut -d':' -f2");
    $MacHigh = exec("cat ".$base_plugin."data/802.conf | grep MacHigh | cut -d':' -f2");
    $MacLow = exec("cat ".$base_plugin."data/802.conf | grep MacLow | cut -d':' -f2");

	$list='
    <div class="title2">802.15.4</div>
    <div class="plugin_content">
        ...
        <div class="right_align">
            <input class="bsave" type="button" value="Save" onclick="save(\'xbee_configuration\',\'output\',\''.$section.'\',\''.$plugin.'\',\'802\')"/>
        </div>
	<div style="width:350px; padding-left: 80px;">
        <div style="background: white; border: 1px solid #898989; padding: 5px;-moz-border-radius: 5px;width:170px;">
            ...
            <button onclick="checkStatus(\'xbee_configuration\',\''.$section.'\',\''.$plugin.'\',\'802\')">Check status</button>
        </div></div>
    </div>';
