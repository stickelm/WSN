...
function hide_mac()
{
    ...
    $('#mac_filter_add_ath0').removeClass('ms_mac');
...
function show_mac()
{
    ...
    $('#mac_filter_add_ath0').addClass('ms_mac');
    //$('#mac_filter_add_ath1').addClass('ms_ip');
    $('#add_mac_filter_ok').show();
}

function add_mac(plugin, seccion)
{
    if(!test_element($('#mac_filter_add_'+$('#interface_selector').val())))
    {
        hide_mac();
        complex_ajax_call('mac_filter_ath0','output', seccion, plugin, 'add_mac','ath0');
    }
}
...
function onchange_key_size()
{
    switch ($('#key_size').val())
    {
        ...
        case "104":
            ...
            $('#cnf_wep_pass').attr("maxlength", "13");
