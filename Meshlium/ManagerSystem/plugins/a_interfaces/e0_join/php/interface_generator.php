...
function generate_interface()
{
    ...
    $list='<div class="title">JOINED NETWORKS</div>';
...
function generate_rules_maker()
{
    ...
    if (!in_array('ppp0', $ifaces))
    {
        $output .= "<h3 class='alarm fleft'>No GPRS Detected</h3>";
        $fail = 1;
    }
    if (!in_array('ath0', $ifaces))
    {
        $output .= "<h3 class='alarm fleft''>No Wifi0 Detected</h3>";
        $fail = 1;
    }
    if (!in_array('ath1', $ifaces))
    {
        $output .= "<h3 class='alarm fleft''>No Wifi1 Detected</h3>";
        $fail = 1;
    }
    if ($fail == 1)
    {
                ...
                <table  cellspacing="15"><tbody>
                    ...
                    <tr>
                        <td>';
                            //if (in_array('ppp0', $ifaces))
                           // {
                               $output.='<img src="'.$url_plugin.'images/j_gprs.png" alt="ppp0" title="GPRS" class="litem click" />';
                           // }
                        $output.='
                        </td>
                        <td rowspan="4">
                        ...
                        <td>';
                           // if (in_array('ppp0', $ifaces))
                           // {
                               $output.='<img src="'.$url_plugin.'images/j_gprs.png" alt="ppp0" title="GPRS" class="ritem click" />';
                           // }
                        $output.='
                        </td>
                    </tr>
