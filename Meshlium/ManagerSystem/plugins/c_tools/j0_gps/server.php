...
function save_interval($data, $path)
{
    exec('echo "'.$data['interval'].'" > '.$path.'interval');
    if ($data['external']=="")
	exec('echo "off" > '.$path.'external');
    else
	exec('echo "'.$data['external'].'" > '.$path.'external');
}
function save_interval2($data, $path)
{
    exec('echo "'.$data['apn'].'" > '.$path.'apn');
    exec('echo "'.$data['http'].'" > '.$path.'http');
    exec('echo "'.$data['port'].'" > '.$path.'port');
    if ($data['firsttime']=="")
	exec('echo "off" > '.$path.'firsttime');
    else
	exec('echo "'.$data['firsttime'].'" > '.$path.'firsttime');
    if ($data['always']=="")
	exec('echo "off" > '.$path.'always');
    else
	exec('echo "'.$data['always'].'" > '.$path.'always');
...
if ($_POST['type']=="save")
{
    ...
    save_interval($hcid_configuration,$base_plugin.'data/');
...
elseif ($_POST['type']=="save2")
{
    exec("sudo remountrw");
    $hcid_configuration=jsondecode($_POST['form_fields']);
    //save_hcid($hcid_configuration,$base_plugin.'data/hcid_conf');
    
    save_interval2($hcid_configuration,$base_plugin.'data/');
...
elseif ($_POST['type']=="startGpsDaemon")
{
    exec("sudo GpsScan.sh &> /dev/null &");
    echo exec("ps ax | grep GpsScan | grep -v grep | wc -l");
}
elseif ($_POST['type']=="downloadFile")
{
    exec("sudo remountrw");
    exec("cp /mnt/user/gps_data/".$_POST['file']." /var/www/ManagerSystem/tmp/.");
    exec("sudo remountro");
}
elseif ($_POST['type']=="viewFile")
{
    exec("sudo remountrw");
    exec("tail -n ".$_POST['num']." /mnt/user/gps_data/".$_POST['file'], $fileArray);
    $fileContent = '';
...
{
    exec("sudo remountrw");
    exec('sudo touch /mnt/user/gps_data/'.$_POST['file']);
    exec("sudo remountro");
    echo make_storedData();
}
...
{
    exec("sudo remountrw");
    exec('sudo rm /mnt/user/gps_data/'.$_POST['file']);
    exec("sudo remountro");
    echo make_storedData();
}
elseif ($_POST['type']=="showMeNow")
{
   /* exec("sudo remountrw");
    ...
    //exec('sudo sh '.$base_plugin.'data/GPSshowNow.sh', $return);
    sleep(4);
     ...
     $nfilas = mysql_num_rows ($consulta);
      if ($nfilas > 0)
      {
         ...
         for ($i=0; $i<$nfilas; $i++)
         {
            ...
            echo '
                    <tr>
                        <td>'.$resultado['ID_frame'].'</td>
                        <td>'.$resultado['date'].'</td>
                        <td>'.$resultado['longitude'].'</td>
                        <td>'.$resultado['latitude'].'</td>
                        <td>'.$resultado['altitude'].'</td>
                        <td>'.$resultado['satellites'].'</td>
                        <td>'.$resultado['speed'].'</td>
                    </tr>
                 ';
         }
...
elseif ($_POST['type']=="checkConnection")
{
   ...
   // Enviar consulta
      $instruccion = "insert into ".$form_data['ExtTable']." values (null, now(), now(), 'a', 'a', 'a', 6, 'a');";
      $consulta = mysql_query ($instruccion, $conexion)
