...
function startGpsDaemon(section,plugin)
{
    submit_data="section="+section+"&plugin="+plugin+"&type=startGpsDaemon";
...
{
    //alert(section+', '+plugin+', '+nonStop+', '+interval+', '+continous+', '+continousStoped);
...
function stopMeNow(section,plugin)
{
    ...
    $('#showMeNowStop').hide();
    continous=0;
    submit_data="section="+section+"&plugin="+plugin+"&type=stopMeNow";
    $.ajax({
        type: "POST",
        url: "index.php",
        data: submit_data,
