...
elseif ($_POST['type']=="startBtDaemon")
{
    ...
    exec("WifiMon.sh &> /dev/null &");
    echo exec("ps ax | grep WifiMon | grep -v grep | wc -l");
}
elseif ($_POST['type']=="downloadFile")
{
    exec("sudo remountrw");
    exec("cp /mnt/user/wifi_data/".$_POST['file']." /var/www/ManagerSystem/tmp/.");
    exec("sudo remountro");
}
elseif ($_POST['type']=="viewFile")
{
    exec("sudo remountrw");
    exec("tail -n ".$_POST['num']." /mnt/user/wifi_data/".$_POST['file'], $fileArray);
    $fileContent = '<div style="border-bottom:1px; border-bottom-style:solid";/><br>';
    foreach ($fileArray as $line) {
        $fileContent .= $line.'<br><br><div style="border-bottom:1px; border-bottom-style:solid";/><br>';
    }
    exec("sudo remountro");
    echo $fileContent;
}
...
{
    exec("sudo remountrw");
    exec('sudo touch /mnt/user/wifi_data/'.$_POST['file']);
    exec("sudo remountro");
    echo make_storedData();
}
...
{
    exec("sudo remountrw");
    exec('sudo rm /mnt/user/wifi_data/'.$_POST['file']);
    exec("sudo remountro");
    echo make_storedData();
}
         ...
         for ($i=0; $i<$nfilas; $i++)
         {
            ...
            echo '
                    <tr>
                        ...
                        <td>'.$resultado['MAC'].'</td>
                        <td>'.$resultado['AP'].'</td>
			<td>'.$resultado['RSSI'].'</td>
			<td>'.$resultado['Vendor'].'</td>
                    </tr>
                 ';
         }
...
elseif ($_POST['type']=="checkConnection")
{
   ...
   // Enviar consulta
      $instruccion = "insert into ".$form_data['ExtTable']." values (null, now(), 'a', 'a','a','a');";
      $consulta = mysql_query ($instruccion, $conexion)
