...
function init() 
{
  ...
  var meshliumLayer = esri.layers.FeatureLayer( config.meshliumUrl + "/0");
  var waspmoteLayer = esri.layers.FeatureLayer( config.meshliumUrl + "/1");
  ...
  dojo.connect(map, "onExtentChange", function(extent,delta,levelChange,lod)
  {
    ...
    dojo.byId("waspmote_x").value = center[0];
    dojo.byId("waspmote_y").value = center[1];
  });
